...
Material Theme Builder export in `ui/theme/Color.kt`, the other two are in
[`ui/theme/ColorThemes.kt`](app/src/main/java/redtoss/creativity/cerebro/ui/theme/ColorThemes.kt).
...
│   ├── data/                   # Strategy, Category, StrategyProvider, editor state, settings
│   └── ui/
...
│       └── theme/              # colour schemes, type scale and spacing scale
...
`ui/theme` holds `Color.kt`, `Theme.kt` and `Type.kt` as a Material Theme Builder
export — the Cosy warm amber palette, with Montserrat Alternates and Roboto Mono
pulled in as downloadable Google Fonts. Those three are regenerated by the tool and
excluded from detekt; the hand-written `ColorThemes.kt` and `Spacing.kt` alongside
them are not.
