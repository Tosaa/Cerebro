...
// Color.kt, Theme.kt and Type.kt in ui/theme are a Material Theme Builder export
// (colour tokens and the generated ColorSchemes). They are regenerated by the tool
// rather than edited by hand, so linting them only produces findings nobody may act
// on. Named individually rather than excluding the whole package: ColorThemes.kt and
// Spacing.kt live there too and are hand-written, so they stay analysed. Applied to
// both the analysis task and the baseline task so the two see the same source set.
val generatedThemeFiles = listOf(
    "**/ui/theme/Color.kt",
    "**/ui/theme/Theme.kt",
    "**/ui/theme/Type.kt",
)
